...
public class EyeTracking : MonoBehaviour
{
    ...
    [SerializeField] int trial_num = 1;
    [SerializeField] bool eye_trac_bool;
    ...
    private void Start()
    {
        ...
        customLogPath = Application.dataPath + "/data/" + SceneManager.GetActiveScene().name + "/"; 
    ...
    void FixedUpdate()   /// ***************************************
    {
        ...
        eye_trac_bool = GameObject.FindGameObjectWithTag("canvas").GetComponent<joystick_move>().eye_trac_bool;

        if (!logging && eye_trac_bool)
        {
            StartLogging();
        ...
        if (logging)
        {
            ...
            {
                LogGazeData(dataSinceLastUpdate[i], eyeMeasurementsSinceLastUpdate[i], save_time); // ************************
            }
    ...
    void LogGazeData(VarjoEyeTracking.GazeData data, VarjoEyeTracking.EyeMeasurements eyeMeasurements, float save_time) //*********************
    {
    ...
    public void StartLogging()
    {
        ...
        //string fileName = string.Format("{0}-{1:00}-{2:00}-{3:00}-{4:00}-{5:00}", now.Year, now.Month, now.Day, now.Hour, now.Minute, trial_num);
        string fileName = string.Format("eye_track_{0}", trial_num);
